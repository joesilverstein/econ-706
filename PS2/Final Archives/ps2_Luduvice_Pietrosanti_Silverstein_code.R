...
setwd("C:/Users/AV/Desktop/ECON706/PS2")
rm(list=ls())
...
# Raw Periodograms:
spectrum(diffStartTS, log="no", main='Raw Periodogram - Starts')
spectrum(diffCompTS, log="no", main='Raw Periodogram - Completions')
...
mySpecPgram(diffCompTS, k, taper=0, log="no", plot=TRUE, bandwidthDisplay=bandwidthComp)
...
# is better 'well-behaved' than the starts series. Below are the plots.

plot(compNSATS)
compNSAtrunc <- window(compNSATS, start = c(2012,1), end = c(2014,12))
plot(compNSAtrunc)
...
bandwidthComp = (4*2+1)/T
par(mfrow=c(1,1))
mySpecPgram(diffCompNSATS, k, taper=0, log="no", plot=TRUE, bandwidthDisplay=bandwidthComp)
...
# By inspection, we see that from 75 lags and on the difference between the estimates
# is not visually significant so we chosse 90 for the sake of parcimony
lagWindow(diffCompNSATS, 75, "Series: diffcompNSATS\nLag Window", layout="alone")
...
# Cross-spectrum
plot(fr,X.p.sp[,1,2],type="l",main="Cross Spectrum", xlab="Frequency",ylab="spectrum")

# Coherence
plot(fr,Mod(X.p.sp[,1,2])^2/( X.p.sp[,1,1]*X.p.sp[,2,2]), 
     type="l",main="Coherence",xlab="Frequency", ylab='')

# Phase
plot(fr,Arg(X.p.sp[,1,2]),type="l",ylim=c(-pi,pi),main="Phase",xlab="Frequency",ylab="Radians")

# Coherence vs. Phase Shift
par(mfrow=c(2,1))
phTU = Arg(X.p.sp[,1,2]) / ((pi/6)*fr)
plot(fr,Mod(X.p.sp[,1,2])^2/( X.p.sp[,1,1]*X.p.sp[,2,2]), 
     type="l",main="Coherence",xlab="Frequency",ylab="spectrum")
plot(fr,phTU,type="l",ylim=c(-2,7),main="Phase Shift (in months)",xlab="Frequency",ylab="Phase Shift (months)")
lines(fr,rep(0,T/2),lty=2)
